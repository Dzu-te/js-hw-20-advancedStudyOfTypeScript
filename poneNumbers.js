...
function getBalanceAsNumber(client) {
    return parseFloat(client.balance.replace(/[^0-9.]/g, ""));
}
const lowBalance = (clients) => {
    const arr = [];
...
const repeatName = (clients) => {
    const duplicateFilter = (client, _, array) => {
    ...
    return duplicateNameClients.sort((next, prev) => next.name < prev.name ? -1 : 1);
};
